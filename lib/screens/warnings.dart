      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            Text('Haemoglobin (Hb): ${patient.hb}',style: const TextStyle(color: Colors.red,fontWeight: FontWeight.bold))
            else
            Text('Haemoglobin (Hb): ${patient.hb}'),
            if(patient.pulse < 60 || patient.pulse > 100)
            Text('Pulse: ${patient.pulse}',style: const TextStyle(color: Colors.red,fontWeight: FontWeight.bold))
            else
            Text('Pulse: ${patient.pulse}'),
            if(patient.spo2 < 95 || patient.spo2 > 100)
            Text('SpO2: ${patient.spo2}',style: const TextStyle(color: Colors.red,fontWeight: FontWeight.bold))
            else
            Text('SpO2: ${patient.spo2}'),
            if(patient.gluc > 126 )
            Text('Glucose: ${patient.gluc}',style: const TextStyle(color: Colors.red,fontWeight: FontWeight.bold))
            else
            Text('Glucose: ${patient.gluc}'),
            if(patient.fhr < 120 || patient.fhr > 160)
            Text('Fetal Heart Rate (FHR): ${patient.fhr}',style: const TextStyle(color: Colors.red,fontWeight: FontWeight.bold))
            else
